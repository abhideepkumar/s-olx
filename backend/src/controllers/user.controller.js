...
import { allowedDomains } from '../../public/alllowedDomains.js';

//register user
export const registerUser = asyncHandler(async (req, res) => {
  const domainPattern = allowedDomains.join('|').replace(/\./g, '\\.'); // Escape dots
  const emailRegex = new RegExp(`^[\\w.-]+@(${domainPattern})$`);
  ...
  if (!emailRegex.test(email)) {
    throw new ApiError(403, 'Your Institute is not yet supported');
  }
...
export const getUserProfile = asyncHandler(async (req, res) => {
  ...
  const userProfile = await user.findById(userId).select('-password').populate('products').populate('posts');
